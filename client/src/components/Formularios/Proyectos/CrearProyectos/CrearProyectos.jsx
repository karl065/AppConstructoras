...
import {cargarProyectos} from '../../../../redux/Actions/ActionsProyectos/ActionsProyectos';
...
const CrearProyectos = ({setModalCargarProyectos}) => {
  const [departamentoSeleccionado, setDepartamentoSeleccionado] = useState('');
  const [ciudadSeleccionada, setCiudadSeleccionada] = useState('');

  const validationSchema = Yup.object({
    nombreProyecto: Yup.string().required('Nombre requerido'),
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, {resetForm}) => {
      cargarProyectos(values);

      resetForm();
      setModalCargarProyectos(false);
      alertSuccess('Proyecto creado con Exito');
    },
  });
